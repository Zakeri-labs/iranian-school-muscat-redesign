...
import { Menu, Phone, Mail, MapPin, LogIn, UserPlus, ChevronDown, MessageCircle } from "lucide-react";
      ...
      <div className="hidden lg:block bg-primary text-primary-foreground text-xs">
        <div className="container mx-auto flex items-center justify-between h-9 px-4">
          <div className="flex items-center gap-5">
            <span className="flex items-center gap-2"><MapPin className="size-3.5" /> {site.addressShort}</span>
          </div>
          <div className="flex items-center gap-5">
            <a href={`mailto:${site.email}`} className="flex items-center gap-2 hover:text-brand transition-colors">
              <Mail className="size-3.5" /> {site.email}
            </a>
            <a href={`tel:${site.phone.replace(/\s/g, "")}`} className="flex items-center gap-2 hover:text-brand transition-colors" dir="ltr">
              <Phone className="size-3.5" /> {site.phone}
            </a>
            <Link to="/portal" className="flex items-center gap-1.5 hover:text-brand transition-colors">
            ...
            item.children ? (
              <DropdownMenu key={item.title}>
                <DropdownMenuTrigger asChild>
                  <button className="inline-flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-foreground hover:bg-secondary hover:text-primary transition-colors">
                ...
                <DropdownMenuContent align="end" className="w-72 p-2">
                  {item.children.map((c) => (
                    <Link
                      key={c.to}
                      to={c.to}
                      className="block rounded-md px-3 py-2 hover:bg-secondary group"
                    >
                      <div className="text-sm font-medium text-primary group-hover:text-brand">{c.title}</div>
              ...
              <Link
                ...
                activeProps={{ className: "rounded-md px-3 py-2 text-sm font-medium text-brand bg-brand-soft" }}
        ...
        <div className="flex lg:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" aria-label="باز کردن منو" className="min-h-11 min-w-11">
            ...
            <SheetContent side="right" className="w-[92vw] sm:w-96 p-0 flex flex-col">
              ...
              <nav className="flex-1 overflow-y-auto p-3" aria-label="منوی موبایل">
                ...
                <a href={`tel:${site.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-secondary" dir="ltr">
                  <Phone className="size-4 text-brand" /> {site.phone}
                </a>
                <a href={`mailto:${site.email}`} className="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-secondary">
              ...
              <div className="p-3 border-t border-border grid grid-cols-2 gap-2">
                ...
                <Button asChild className="bg-brand hover:bg-brand/90 text-brand-foreground" onClick={() => setOpen(false)}>
                  <Link to="/admissions">ثبت‌نام</Link>
                </Button>
              </div>
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-40 lg:hidden bg-background/95 backdrop-blur border-t border-border">
      <div className="grid grid-cols-4">
        ...
        <Link to="/news" className="flex flex-col items-center justify-center gap-1 py-2 text-[11px] min-h-14 text-foreground hover:bg-secondary">
          <svg className="size-5 text-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
          اطلاعیه‌ها
        </Link>
      </div>
